...
import { ShieldCheck, AlertTriangle, Download, Upload, X, CheckCircle, XCircle, Eye, RefreshCw, User } from "lucide-react";
    ...
    <AdminPageShell withSidebar searchPlaceholder="Search certificates...">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">Manage Admins, Trainers and Trainees</p>
        </div>
        <div className="flex gap-3">
          <Dialog open={showBulkUpload} onOpenChange={setShowBulkUpload}>
            <DialogTrigger asChild>
              <Button>
                <User className="h-4 w-4" /> Create New User
              </Button>
            </DialogTrigger>
      ...
      <div className="mt-8 flex flex-wrap gap-4">
        <div className="flex gap-2">
          ...
          >
            Inactive ({certificates.filter(c => c.status === 'revoked').length})
          </Button>
        </div>
        <Input
