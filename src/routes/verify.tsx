...
export default function Verify() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background" style={{ backgroundColor: '#f7f8f9' }}>
      {user ? <TopNav searchPlaceholder="Search certificates..." /> : <Navbar />}
          ...
          {verificationStatus && certificateData && (
            <aside className="space-y-6">
              <div className="rounded-2xl bg-card p-6 shadow-[var(--shadow-card)]">
                <div className="flex items-start gap-3">
                  <div className="grid h-10 w-10 place-items-center rounded-md bg-energy/20"><Award className="h-5 w-5 text-energy-foreground" /></div>
                  <h3 className="text-lg font-bold">Certificate Details</h3>
                </div>
                <div className="mt-5 space-y-4 text-sm">
                  <div className="flex items-center justify-between">
                    <span>Status</span>
                    <span className={`font-bold ${verificationStatus === 'revoked' ? 'text-destructive' : 'text-primary'}`}>
                      {verificationStatus === 'revoked' ? 'Revoked' : 'Active'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>Certificate ID</span>
                    <span className="font-bold font-mono text-xs">{certificateData.certificate_number}</span>
                  </div>
                  {certificateData.qr_code && (
                    <div className="pt-3 border-t">
                      <p className="text-xs text-muted-foreground mb-2">QR Code Available</p>
                      <img src={certificateData.qr_code} alt="QR Code" className="w-full h-auto max-w-[120px]" />
                    </div>
                  )}
                </div>
              </div>
              <div className="relative h-64 overflow-hidden rounded-2xl shadow-[var(--shadow-card)]">
                ...
                <div className="absolute bottom-0 p-5 text-primary-foreground">
                  ...
                  <p className="text-sm text-primary-foreground/85">Gokly Oil & Gas Institute</p>
                </div>
              </div>
            </aside>
          )}
